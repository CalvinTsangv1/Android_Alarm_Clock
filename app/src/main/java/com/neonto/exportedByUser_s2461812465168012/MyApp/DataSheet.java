...
public class DataSheet {
    ...
    protected void loadRowsWithPersistenceName(String persistenceName) {
        ...
        try {
            ...
            // de-stringify json objects
            for (HashMap<String, HashMap<String, Object>> row : mRows) {
                for (HashMap<String, Object> col : row.values()) {
                    String type = (String) col.get("type");
                    if (type.equals("json")) {
                        Object obj = new org.json.JSONTokener((String) col.get("value")).nextValue();
                        col.put("value", obj);
                    }
                }
            }
    ...
    public int RowsIndexCheck(){
        return mRows.size();
    }


    public String CheckRows(int rowIndex,Object rowObject) {
        HashMap<String, HashMap<String, Object>> row = mRows.get(rowIndex);
        HashMap<String, Object> val;
        val = row.get(rowObject);
    ...
    public void saveRowWithValues(HashMap<String, Object> values) {
        updateRowImpl(mRows.size(), values);
    }

    public void updateRow(int rowIndex, HashMap<String, Object> values) {
        updateRowImpl(rowIndex, values);
    }

    private void updateRowImpl(int rowIndex, HashMap<String, Object> values) {
    ...
    public void save() {
        ...
        try {
            ...
            // json objects aren't serializable, let's stringify them
            for (HashMap<String, HashMap<String, Object>> row : mRows) {
                for (HashMap<String, Object> col : row.values()) {
                    String type = (String) col.get("type");
                    if (type.equals("json")) {
                        Object obj = col.get("value");
                        col.put("value", obj.toString());
                    }
                }
            }
