...
public class RecordStopwatchListItemImageView extends FrameLayout {



    private ImageView m_image;
    // This value represents scaling applied to the image by Neonto Studio on export.
    private float m_contentOriginalScale = 0.153333333f;
    

    private Matrix m_contentTransform;

    private boolean m_imageOverriden = false;

    public RecordStopwatchListItemImageView(Context context) {
        super(context);
        init(null, 0);
        createViews(context);
    }
    
    public RecordStopwatchListItemImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
        createViews(context);
    }
    
    public RecordStopwatchListItemImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void createViews(Context context) {
        ...
        d = getResources().getDrawable(R.drawable.record_stopwatch_list_item_image);
        ...
        // Needed by shape mask to handle alpha properly.
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        float maskCornerRadius = Math.min(w, h) * 0.5f;
        
        final Path maskPath = new Path();
        maskPath.addRoundRect(new RectF(0, 0, w, h), maskCornerRadius, maskCornerRadius, Path.Direction.CW);
